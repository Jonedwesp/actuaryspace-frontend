...
export async function handler(event) {
  ...
  if (error) return json(400, { ok: false, error, qs });
  if (!code) return json(400, { ok: false, error: "Missing ?code", qs });

  if (!CLIENT_ID || !CLIENT_SECRET || !REDIRECT_URI) {
    return json(500, {
      ok: false,
      error:
        "Missing GOOGLE_CLIENT_ID or GOOGLE_CLIENT_SECRET or GOOGLE_OAUTH_REDIRECT",
      have: {
  ...
  try {
    ...
    // Non-HttpOnly flag cookie (lets UI show "Connected" without exposing tokens)
    cookies.push(
    ...
    return {
      ...
      multiValueHeaders: {
        "Set-Cookie": cookies, // ✅ Netlify/Lambda-compatible
      },
      body: "",
    };
  } catch (e) {
