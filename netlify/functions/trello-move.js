...
export const handler = async (event) => {
  ...
  // Helper to make Trello requests SAFELY
    ...
    const req = https.request(options, (res) => {
      ...
      res.on("end", () => {
          ...
          try {
              ...
              // If Trello returns plain-text (like "invalid id"), catch it without crashing
  ...
  try {
    ...
    // Default to 'bottom' if list fetch fails
    let newPos = "bottom"; 
    ...
    if (moveRes.status >= 200 && moveRes.status < 300) {
      ...
      console.error("Trello API Rejected Move:", moveRes.data);
