...
exports.handler = async (event) => {
  ...
  const safeSubject = subject || "New Message";
    ...
    const sendRes = await googleRequest("https://gmail.googleapis.com/gmail/v1/users/me/messages/send", "POST", {
      "Authorization": `Bearer ${token}`,
      "Content-Type": "application/json"
    }, JSON.stringify({ raw: encodedMail }));

    if (!sendRes.ok) {
      return { 
        statusCode: sendRes.status, 
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ ok: false, error: sendRes.data?.error?.message || "Gmail API rejection" }) 
      };
    }
    ...
    return { 
      statusCode: 200, 
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ ok: true, messageId: sendRes.data.id }) 
    };
  } catch (err) {
    return { 
      statusCode: 500, 
